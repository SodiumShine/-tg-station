...
/mob/new_player/proc/IsJobAvailable(rank)
	...
	if(!job.player_old_enough(src.client))
		return 0
	if(config.enforce_human_authority && ((rank in command_positions) || (rank in security_positions)) && client.prefs.pref_species.id != "human")
		return 0
	return 1
...
/mob/new_player/proc/ViewManifest() // SHINE ADD
		var/dat = "<html><body>"
